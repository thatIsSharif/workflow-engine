...
export default function CancellationListPage() {
  ...
  useEffect(() => {
    cancellationApi.list().then(setItems).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/cancellation/new" className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors">+ New Cancellation</Link>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {items.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-4xl mb-3">{MODULE_ICONS.CANCELLATION}</p>
            <p className="text-gray-500">No cancellation requests yet.</p>
            <Link href="/cancellation/new" className="inline-block mt-2 text-indigo-600 hover:underline">Create one</Link>
          </div>
        ) : (
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => router.push(`/cancellation/${item.id}`)}>
